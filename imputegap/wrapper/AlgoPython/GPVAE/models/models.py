...
# https://arxiv.org/abs/1907.04155
# ===============================================================================================================
"""
...
class BandedJointEncoder(tf.keras.Model):
    ...
    def __call__(self, x):
        ...
        with tf.device('/cpu:0'):
            ...
            prec_tril = tf.zeros(prec_sparse.dense_shape, dtype=tf.float32) + tf.sparse.to_dense(prec_sparse)
...
class GP_VAE(HI_VAE):
    ...
    def kl_divergence(self, a, b):
        ...
        if is_diagonal(a.scale) and is_diagonal(b.scale):
            ...
            print("Type", type(a.stddev()), type(b.stddev))
